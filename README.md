...
- **v0.7 (운용개선)** — 나무 오탐 줄이기. **재학습 없이** 나무 NMS 중복제거 + 클래스별 threshold로 **4-도메인 FPPI 절반 이하**(예: 청라 8.4→3.7), mAP 불변. 가중치는 v0.5 그대로.
  - **운용점 2종 (나무 threshold만 다름, 6/9 4-도메인 재측정)** — 같은 v0.5 가중치, FP↔recall 트레이드오프를 골라 쓰는 후처리:

    | 도메인 | 전체 FPPI<br>(베이스→F1→정밀) | 나무 FPPI | 나무 recall |
    |---|---|---|---|
    | 청라10m | 8.43 → 3.65 → **2.21** | 4.41 → 2.14 → **0.69** | 18.0% → 13.5% → 5.6% |
    | 용인하남15m | 9.55 → 3.19 → 3.19 | 0.01 → 0.00 → 0.00 | (나무 GT 0) |
    | 택배15m | 7.77 → 2.61 → **2.53** | 0.17 → 0.12 → **0.04** | (나무 GT 0) |
    | 농약 | 5.00 → 2.20 → **1.87** | 0.93 → 0.55 → **0.23** | 46.7% → 39.7% → 27.3% |

    **F1레버**(tree τ=0.325) = 나무 recall 보존 · **정밀레버**(tree τ=0.40) = FPPI 최저. 모델 mAP@0.5(풀캐시, 레버 무관 불변): 청라 29.3 / 용인하남 25.1 / 택배 43.6 / 농약 43.6. ⚠️ 레버는 모든 클래스 τ를 0.3 위로 올리는 정밀도-우선 운용점이라 macro recall은 하락(청라 41.6→32~33%) — mAP는 불변, recall 하락은 더 높은 정밀도 점을 택한 대가.
